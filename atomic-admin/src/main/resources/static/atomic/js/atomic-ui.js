...
(function ($) {
    $.extend({
        ...
        operate: {
        	// 提交数据
        	submit: function(url, type, dataType, data, tableId) {
            	var config = {
        	        url: url,
        	        type: type,
        	        dataType: dataType,
        	        ...
        	        success: function(result) {
        	        	$.operate.ajaxSuccess(result, tableId);
        	        }
        	    };
        	    $.ajax(config)
            },
            // post请求传输
            post: function(url, data, tableId) {
            	$.operate.submit(url, "post", "json", data, tableId);
            },
            // get请求传输
            get: function(url) {
			...
			addWithParams: function(id){
                $.modal.open("添加" + $.table._option.modalName, $.operate.addUrlWithParams(id));
			},
            // 添加信息，以tab页展现
            addTab: function (id) {
            ...
            // 保存信息 刷新表格
            save: function(url, data, tableId) {
            	var config = {
        	        url: url,
        	        type: "post",
        	        dataType: "json",
        	        ...
        	        success: function(result) {
        	        	$.operate.successCallback(result, tableId);
        	        }
        	    };
        	    $.ajax(config)
            },
            ...
            // 保存结果弹出msg刷新table表格
            ajaxSuccess: function (result, tableId) {
            ...
            // 成功回调执行事件（父窗体静默更新）
            successCallback: function(result, tableId) {
                if (result.code == web_status.SUCCESS) {
                	var parent = window.parent;
                    if (parent.$.table._option.type == table_type.bootstrapTable) {
                        $.modal.close();
                        parent.$.modal.msgSuccess(result.msg);
                        parent.$.table.refresh(tableId);
            ...
            successTabCallback: function(result) {
                if (result.code == web_status.SUCCESS) {
    	            ...
    	            var $contentWindow = $('.Atomic_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
    	            $.modal.close();
